...
var (
	...
	AccidentalMana          = []string{"max", "max"} // Defines node which will be used: min, max or random
	...
	AdversaryMana           = []float64{}            // Adversary nodes mana in %, e.g. '10 10' Special values: -1 nodes should be selected randomly from weight distribution, 0 weight=min(weightDistribution), 100 weight=max(weightDistribution). SimulationTarget must be 'DS'
