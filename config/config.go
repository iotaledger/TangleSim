...
var (
	...
	AdversaryTypes          = []int{1, 2, 3, 4}            // Defines group attack strategy, one of the following: 0 - honest node behavior, 1 - shifts opinion, 2 - keeps the same opinion. SimulationTarget must be 'DS'
	AdversaryMana           = []float64{5, 5, 5, 5}        // Adversary nodes mana in %, e.g. '10 10' Special values: -1 nodes should be selected randomly from weight distribution, 0 weight=min(weightDistribution), 100 weight=max(weightDistribution). SimulationTarget must be 'DS'
	AdversaryNodeCounts     = []int{10, 1, 1, 1}           // Defines number of adversary nodes in the group. Leave empty for default value: 1. SimulationTarget must be 'DS'
	AdversaryInitColors     = []string{"R", "G", "B", "B"} // Defines initial color for adversary group, one of following: 'R', 'G', 'B'. Mandatory for each group. SimulationTarget must be 'DS'
)
