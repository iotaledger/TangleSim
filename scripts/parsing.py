...
def parse_cf_file(fn, var):
    ...
    # node, msgid, time
    # group by msgID.avg time - regardless of node
    # group by node avg time
    config_fn = re.sub('cf', 'aw', fn)
    ...
    v = str(c[var])
    N = c["NodesCount"]
    data['Confluence Time'] = data['Confluence Time'] / float(c["DecelerationFactor"])
